...
DEBUG_INFO   = True #general info messages
        ...
        while True:
            ...
            if msg_str:
                ...
                if msg_json['TYPE'] == 'TEMP':
                    ...
                    self.routing_table[msg_json['SRC']] = msg_json['LSRC']
                    akn_dst = msg_json['SRC']
                    if (DEBUG_ROUTE):
                        print(self.env.now,':', self.id ,'routing table :' , str(self.routing_table))
                    self.sqnr += 1
                    msg_json = {}
                    msg_json['TYPE'] = 'AKN'
                    msg_json['SRC']  = self.id
                    msg_json['DST']  = akn_dst 
                    msg_json['LSRC'] = self.id
                    msg_json['LDST'] = self.routing_table[akn_dst]
                    msg_json['SEQ']  = self.sqnr
                    msg_json['DATA'] = "sensor:" + str(msg_json['SRC'])
                    msg_str = json.dumps( msg_json )
                    if (DEBUG_INFO):
                        print(self.env.now,':', self.id ,'sending acknowledgement' , msg_str)
                    self.send(msg_json['LDST'],msg_str)
			...
			if msg_str:
				...
				if msg_json['TYPE'] == 'JOIN':
					...
					if self.join_node == 0:
						...
						msg_json['LSRC'] = self.id
						msg_json['LDST'] = self.join_node
						msg_json['DATA'] = str(msg_json['DATA']) + ' via ' + str(self.id)
						msg_str = json.dumps( msg_json )
						if (DEBUG_ROUTE):
							print(self.env.now,':', self.id ,'routing' , msg_str)
						self.send(msg_json['LDST'],msg_str)
				elif msg_json['TYPE'] == 'AKN':
					if (msg_json['DST'] == self.id):
						print(self.env.now,':',  self.id,'receive acknowledgement from', msg_json['DATA'])
					else:
						try:
							msg_json['LSRC'] = self.id
							msg_json['LDST'] = self.routing_table[msg_json['DST']]
							msg_json['DATA'] = str(msg_json['DATA']) + ' via ' + str(self.id)
							msg_str = json.dumps( msg_json )
							if (DEBUG_ROUTE):
								print(self.env.now,':', self.id ,'routing' , msg_str)
							...
							print(self.env.now,':', self.id ,'cannot route message to DST:', msg_json['DST'])
