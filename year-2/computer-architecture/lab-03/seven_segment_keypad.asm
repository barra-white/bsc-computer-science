...
#====================================
main: 		
		addi $s0,$zero, 1		#prepare register for iterating over rows
		li $s1, 16			# load register with 8 for comparing
		li $s2, 2			# load with 2 to iterate over rows
		li $t7, 113			# load register to prepare to exit loop when f is pressed
		addi $a0, $0,0 		 
		j init7SegDisp
main2:
		jal getPressedKey
		add $a0,$v0,$zero
		jal Num2SegDisp
		jal getPressedKey		#get pressed key of second digiy
		add $a0,$v0,$zero
		jal Num2SegDisp2		#2nd digit to segDisp
		bne $t7, $t1, main2
...
FstRow:	
		beq $s1, $s0, resetRow		# after iterating over final row, reset to first row
		sb $s0, 0x12($t0)  	# command first row scanning at 0xFFFF0012
		lb  $t2, 0x14($t0)  	# read  0xFFFF0014 for pressed keys 
		bne $t2, $zero, KeyPress # check any pressed keys [zero --> nothing pressed]
		mul $s0, $s2, $s0	# mul $t4 by 2 to iterate over next row
		j  FstRow  		#scan next row 
		
resetRow:
		li $s0, 1		# reset register to scan over first rowagain
		j FstRow		# start loop again
