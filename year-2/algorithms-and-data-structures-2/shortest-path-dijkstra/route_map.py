...
class RouteMap:
    ...
    def __init__(self):
        ...
        # dict with element as key and vertex as value
        self._elements = dict()

    def __str__(self):
        vertices = int(self.num_vertices())
        edges = int(self.num_edges())
        if (vertices > 100) or (edges > 100):
            return 'Empty graph'
    ...
    def num_vertices(self):
        ...
        return int(len(self._structure))
    ...
    def num_edges(self):
        ...
        return int(num //2)     #divide by 2, since each edge appears in the
    ...
    def sp(self, v, w, APQType: bool):
        """ Return a shortest path from v to w. """
        closed = self.dijkstra(v, None, APQType)[0]
        path = [w]
        while w:
            cost, pred = closed[w][0], closed[w][1]
            path.append(w, cost)
            w = pred
        revPath = path[::-1]
        for v in revPath:
            print(f'\nType: W\tLatitute: {v.element().getValue()[0]}\tLongitude: {v.element().getValue()[1]}\tElement: {v.element().getIndex()}\tCost: {v.element().getKey()}')
        return revPath
...
def graphreader(filename):
     ...
     while entry == 'Node\n':
          ...
          gpsLine = file.readline().split()
          gps = (float(gpsLine[1]), float(gpsLine[2]))
          vertex = route.add_vertex(nodeid, gps)
          entry = file.readline() #either 'Node' or 'Edge'
...
g = graphreader('simpleroute.txt')
g.sp(g.get_vertex_by_label(1), g.get_vertex_by_label(3), True)
